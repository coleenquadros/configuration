# Hypershift provisioning with app-interface

<!-- TOC -->

- [Hypershift provisioning with app-interface](#hypershift-provisioning-with-app-interface)
...
TODO: deploy ACM via SAAS file - currently we need to deploy upstream ACM/MCE because we rely on features and fixes only present there. Right now, ACM upstream can't be updated but must be reinstalled. So while the templates for ACM have been written already, they can't yet be used by app-interface and updates are done manually.

Tracking ticket from AppSRE side: https://issues.redhat.com/browse/APPSRE-4619
