...
- `max_records`: A upper limit of total number of the records allowed, it should be large then existing number of records. If unset, its value is default to the `cloudflareDNSZoneMaxRecords` setting in [App Interface settings](https://gitlab.cee.redhat.com/service/app-interface/-/blob/master/data/app-interface/app-interface-settings.yml). This requires App SRE's approval, please see [Performance Section](#performance).
